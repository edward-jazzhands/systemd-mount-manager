...
class TextualApp(App[None]):
    ...
    BINDINGS = [
        ...
        Binding("f4", "show_context", "Show context"),
    ]

    CSS_PATH = "styles.tcss"
    TITLE = "Systemd Mount Manager"

    def __init__(self, context: logic.core.StartupResult) -> None:
        # dev mode will activate the css watching feature
        super().__init__(watch_css=context.dev)
    ...
    def action_show_context(self) -> None:
        logger.debug(f"Startup results: {self.context}")


def tui_run(context: logic.core.StartupResult) -> None:

    app = TextualApp(context)
    app.run()
